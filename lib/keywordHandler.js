...
class KeywordHandler {
    ...
    handleSql(line, foundKeyword) {
        ...
        if (match !== null) {
            this.diagnostics.push(util_1.Util.createDiagnostic(util_1.Util.createRange(match[1].length, "sql".length, foundKeyword.range.start.line), messageUtil_1.lineFeedRequired("sql")));
        }
    }
    /**
    ...
    handleIf(line, foundKeyword) {
        ...
        if (ifCondition.trim() === "") {
            this.diagnostics.push(util_1.Util.createDiagnostic(foundKeyword.range, "If condition can not be empty"));
            return;
        }
        try {
            ...
            this.diagnostics.push(util_1.Util.createDiagnostic(util_1.Util.createRange(line.indexOf(ifCondition), ifCondition.length, foundKeyword.range.start.line), err.message));
        }
    }
    handleScript(line, foundKeyword) {
        ...
        if (match !== null) {
            this.diagnostics.push(util_1.Util.createDiagnostic(util_1.Util.createRange(match[1].length, "script".length, foundKeyword.range.start.line), messageUtil_1.lineFeedRequired("script")));
        }
    }
}
