...
class Formatter {
    ...
    checkSign() {
        ...
        if (spacesBefore !== " ") {
            this.edits.push(vscode_languageserver_types_1.TextEdit.replace(util_1.Util.createRange(declaration.length, spacesBefore.length, this.currentLine), " "));
        }
        if (spacesAfter !== " ") {
            const start = line.indexOf(sign) + sign.length;
            this.edits.push(vscode_languageserver_types_1.TextEdit.replace(util_1.Util.createRange(start, spacesAfter.length, this.currentLine), " "));
        }
    }
    /**
