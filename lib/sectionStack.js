(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "vscode-languageserver-types", "./setting", "./textRange", "./util", "./languageService", "./resourcesProviderBase"], factory);
    }
    ...
    class SectionStackNode {
        constructor(range) {
            ...
            const deps = resourcesProviderBase_1.ResourcesProviderBase.getRequiredSectionSettingsMap(settingsMap).get(this.name);
    ...
    class SectionStack {
        ...
        checkAndGetDepth(sectionRange) {
            ...
            let actualDepth = resourcesProviderBase_1.ResourcesProviderBase.sectionDepthMap[section];
            let error = null;
            if (actualDepth == null) {
                error = this.createErrorDiagnostic(sectionRange, `Unknown section [${section}].`);
            }
            else if (actualDepth > expectedDepth) {
                let canBeInherited = resourcesProviderBase_1.ResourcesProviderBase.inheritableSections.has(section);
                if (canBeInherited && expectedDepth > 0) {
                    actualDepth = expectedDepth;
                }
                else {
                    let errorMessage = `Unexpected section [${section}]. `;
                    let expectedSections = Object.entries(resourcesProviderBase_1.ResourcesProviderBase.sectionDepthMap)
