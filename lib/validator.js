...
class Validator {
    ...
    addToSettingArray(variable, array) {
        ...
        if (declaredAbove !== undefined) {
            ...
            this.result.push(util_1.Util.repetitionDiagnostic(range, declaredAbove, variable));
        }
        else {
    ...
    addToStringArray(array) {
        ...
        if (array.includes(variable)) {
            this.result.push(util_1.Util.createDiagnostic(this.createRange(indent.length, variable.length), `${variable} is already defined`));
        }
        else {
            result.push(variable);
        }
    ...
    addToStringMap(map, key) {
        ...
        const [, indent, variable] = this.match;
        if (util_1.Util.isInMap(variable, map)) {
            const startPosition = this.match.index + indent.length;
            this.result.push(util_1.Util.createDiagnostic(this.createRange(startPosition, variable.length), `${variable} is already defined`));
        }
        else {
    ...
    checkAliases() {
        this.deAliases.forEach((deAlias) => {
            if (!this.aliases.includes(deAlias.text)) {
                this.result.push(util_1.Util.createDiagnostic(deAlias.range, messageUtil_1.unknownToken(deAlias.text)));
            }
        });
    }
    /**
    ...
    checkEnd(expectedEnd) {
        ...
        if (!this.areWeIn(expectedEnd)) {
            this.result.push(util_1.Util.createDiagnostic(this.foundKeyword.range, messageUtil_1.noMatching(this.foundKeyword.text, expectedEnd)));
        }
        else {
            const index = this.keywordsStack.findIndex((keyword) => keyword.text === expectedEnd);
            this.keywordsStack.splice(index, 1);
            this.result.push(util_1.Util.createDiagnostic(this.foundKeyword.range, `${expectedEnd} has finished before ${lastKeyword}`));
        }
    }
    /**
    ...
    checkExcludes(setting) {
        ...
        for (const item of this.currentSettings) {
            if (setting.excludes.includes(item.displayName)) {
                const range = this.createRange(indent.length, name.length);
                this.result.push(util_1.Util.createDiagnostic(range, `${setting.displayName} can not be specified simultaneously with ${item.displayName}`));
            }
        }
    }
    checkFreemarker() {
        ...
        if (this.match !== null) {
            this.result.push(util_1.Util.createDiagnostic(this.createRange(this.match.index, this.match[0].length), `Freemarker expressions are deprecated.\nUse a native collection: list, csv table, var object.` +
    ...
    checkRequredSettingsForSection() {
        ...
        required: for (const options of this.requiredSettings) {
            ...
            if (util_1.Util.isAnyInArray(optionsNames, this.currentSettings.map(s => s.name))) {
                continue;
            }
            for (const array of this.parentSettings.values()) {
                // Trying to find in this section parents
                if (util_1.Util.isAnyInArray(optionsNames, array.map(s => s.name))) {
                    continue required;
                }
            }
        ...
        for (const option of notFound) {
            this.result.push(util_1.Util.createDiagnostic(this.currentSection.range, `${option} is required`));
        }
    ...
    checkRepetition(setting) {
        ...
        if (this.areWeIn("if")) {
            ...
            if (declaredAbove !== undefined) {
                ...
                this.result.push(util_1.Util.repetitionDiagnostic(range, declaredAbove, setting));
                return;
            }
        }
        else {
    ...
    diagnosticForLeftKeywords() {
        for (const nestedConstruction of this.keywordsStack) {
            ...
            this.result.push(util_1.Util.createDiagnostic(nestedConstruction.range, messageUtil_1.noMatching(nestedConstruction.text, `end${nestedConstruction.text}`)));
        }
    }
    /**
    ...
    eachLine() {
        ...
        this.match !== null ||
            ...
            if (this.match !== null) {
                this.result.push(util_1.Util.createDiagnostic(this.createRange(this.match[1].length, this.match[2].length), "Section tag is unclosed"));
            }
        }
    }
    /**
    ...
    getSettingCheck() {
        ...
        if (setting === undefined) {
            ...
            this.result.push(util_1.Util.createDiagnostic(this.createRange(this.match[1].length, settingName.length), message));
            return undefined;
        }
    ...
    handleCsv() {
        ...
        if (regExpressions_1.CSV_INLINE_HEADER_PATTERN.exec(line)) {
            ...
            if (match !== null) {
                ...
                this.result.push(util_1.Util.createDiagnostic(this.foundKeyword.range, messageUtil_1.getCsvErrorMessage(line)));
            }
        }
        this.addToStringMap(this.variables, "csvNames");
        this.csvColumns = (header === null) ? 0 : util_1.Util.countCsvColumns(header);
    }
    /**
    ...
    handleElse() {
        ...
        if (message !== undefined) {
            this.result.push(util_1.Util.createDiagnostic(this.foundKeyword.range, message));
        }
    }
    /**
            ...
            if (collection !== "") {
                ...
                if (!varName) {
                    try {
                        ...
                        this.result.push(util_1.Util.createDiagnostic(this.createRange(start, collection.length), "Incorrect collection declaration."));
                    }
                }
                else if (!util_1.Util.isInMap(varName, this.variables)) {
                    const message = messageUtil_1.unknownToken(varName);
                    const start = line.lastIndexOf(varName);
                    this.result.push(util_1.Util.createDiagnostic(this.createRange(start, varName.length), message));
                }
            }
            else {
                const start = this.match[0].indexOf("in");
                this.result.push(util_1.Util.createDiagnostic(this.createRange(start, "in".length), "Empty 'in' statement"));
            }
    ...
    handleSettings() {
        ...
        if (this.currentSection === undefined || !/(?:tag|key)s?/.test(this.currentSection.text)) {
            ...
            if (this.isAllowedWidget(setting)) {
                this.result.push(util_1.Util.createDiagnostic(this.createRange(indent.length, name.length), messageUtil_1.settingNameInTags(name), vscode_languageserver_types_1.DiagnosticSeverity.Information));
            }
        }
    }
    /**
    ...
    handleRegularSetting() {
        ...
        if (setting.deprecated) {
            this.result.push(util_1.Util.createDiagnostic(setting.textRange, setting.deprecated, vscode_languageserver_types_1.DiagnosticSeverity.Warning));
        }
        if (!this.isAllowedInSection(setting)) {
            this.result.push(util_1.Util.createDiagnostic(setting.textRange, messageUtil_1.illegalSetting(setting.displayName), vscode_languageserver_types_1.DiagnosticSeverity.Error));
        }
        ...
        if (match != null && match[2]) {
            ...
            if (settingName && !this.foundKeyword && /^\w+(\s.*\w)+$/.test(settingName)) {
                ...
                if (this.currentSection.text === "tags") {
                    if (!/^["].+["]$/.test(settingName)) {
                        this.result.push(util_1.Util.createDiagnostic(range, messageUtil_1.tagNameWithWhitespaces(settingName), vscode_languageserver_types_1.DiagnosticSeverity.Warning));
                    }
                }
                else if (this.currentSection.text !== "properties") {
                    this.result.push(util_1.Util.createDiagnostic(range, messageUtil_1.settingsWithWhitespaces(settingName), vscode_languageserver_types_1.DiagnosticSeverity.Warning));
                }
            }
        }
    }
    ...
    spellingCheck() {
        ...
        if (word === "tag") {
            this.result.push(util_1.Util.createDiagnostic(range, messageUtil_1.deprecatedTagSection, vscode_languageserver_types_1.DiagnosticSeverity.Warning));
        }
    }
    /**
    ...
    validateCsv() {
        const line = this.config.getCurrentLine();
        const columns = util_1.Util.countCsvColumns(line);
        if (columns !== this.csvColumns && !/^[ \t]*$/m.test(line)) {
            this.result.push(util_1.Util.createDiagnostic(this.createRange(0, line.length), `Expected ${this.csvColumns} columns, but found ${columns}`));
        }
    }
    /**
    ...
    validateFor() {
        ...
        while (this.match !== null) {
            ...
            while (this.match !== null) {
                ...
                if (!util_1.Util.isInMap(variable, this.variables)) {
                    const position = startPosition + this.match.index;
                    const message = messageUtil_1.unknownToken(variable);
                    this.result.push(util_1.Util.createDiagnostic(this.createRange(position, variable.length), message));
                }
                this.match = varRegexp.exec(substr);
            }
            this.match = atRegexp.exec(line);
    ...
    getSetting(name) {
        ...
        return languageService_1.LanguageService.getResourcesProvider().getSetting(name, range);
    }
    checkUrlPlaceholders() {
        ...
        if (placeholderRange) {
            ...
            if (missingPhs.length > 0) {
                this.result.push(util_1.Util.createDiagnostic(placeholderRange.range, `Missing placeholders: ${missingPhs.join(", ")}.`, vscode_languageserver_types_1.DiagnosticSeverity.Error));
            }
            let unnecessaryPhs = phSectionSettings.filter(s => !phs.includes(s.name)).map(s => s.name);
            if (unnecessaryPhs.length > 0) {
                this.result.push(util_1.Util.createDiagnostic(placeholderRange.range, `Unnecessary placeholders: ${unnecessaryPhs.join(", ")}.`, vscode_languageserver_types_1.DiagnosticSeverity.Warning));
            }
        }
    }
    /**
    ...
    createRange(start, length) {
        return util_1.Util.createRange(start, length, this.config.currentLineNumber);
    }
}
