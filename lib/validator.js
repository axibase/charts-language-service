...
class Validator {
    ...
    checkMultipleSetting(setting) {
        ...
        if (!validSingle && !validMultiple) {
            this.result.push(util_1.createDiagnostic(textRange, `${displayName} should be a comma separated list. For example, ${example}`, vscode_languageserver_types_1.DiagnosticSeverity.Error));
        }
    }
    /**
